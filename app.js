...
const mat=(c,o={})=>new THREE.MeshStandardMaterial({color:c,roughness:.85,...o});const white=mat('#eeeee4'),siding=mat('#d9dbcf'),trim=mat('#f7f6ed'),red=mat('#78332e'),wood=mat('#815142'),stone=mat('#a49172'),dark=mat('#383e3c'),glass=mat('#52696b',{metalness:.25,roughness:.28}),roof=mat('#72685b'),grass=mat('#8a9d74',{roughness:1}),asphalt=mat('#666b6d',{roughness:.95}),screen=mat('#343e3b',{transparent:true,opacity:.53,side:THREE.DoubleSide,depthWrite:false});
for(const [material,kind,repeat,bump] of [[roof,'roof',[5,4],.025],[stone,'paving',[2,2],.016]]){material.map=surfaceTexture(renderer,kind,repeat);material.bumpMap=material.map;material.bumpScale=bump;}
...
// Fine value noise as a repeating canvas texture: mottled lawn, grainy asphalt and bark.
function noiseMap(base,spread,repeat,{grain=1,streaks=0}={}){const c=document.createElement('canvas');c.width=c.height=256;const g=c.getContext('2d');const img=g.createImageData(256,256),d=img.data;let seed=7;const rnd=()=>{seed=(seed*16807)%2147483647;return seed/2147483647};const cells=[];for(let i=0;i<64*64;i++)cells.push(rnd());const smooth=(x,y)=>{const cx=Math.floor(x),cy=Math.floor(y),fx=x-cx,fy=y-cy,at=(a,b)=>cells[((b&63)*64)+(a&63)];const u=fx*fx*(3-2*fx),v=fy*fy*(3-2*fy);return (at(cx,cy)*(1-u)+at(cx+1,cy)*u)*(1-v)+(at(cx,cy+1)*(1-u)+at(cx+1,cy+1)*u)*v};for(let y=0;y<256;y++)for(let x=0;x<256;x++){const n=smooth(x/16,y/16)*.55+smooth(x/4,y/4)*.3+rnd()*.15*grain;const k=(n-.5)*spread+(streaks?Math.sin(y*.9)*streaks:0);const i=(y*256+x)*4;d[i]=base[0]+k*base[0];d[i+1]=base[1]+k*base[1];d[i+2]=base[2]+k*base[2];d[i+3]=255}g.putImageData(img,0,0);const t=new THREE.CanvasTexture(c);t.colorSpace=THREE.SRGBColorSpace;t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(repeat,repeat);t.anisotropy=renderer.capabilities.getMaxAnisotropy();return t}
grass.map=noiseMap([132,150,108],.5,14);asphalt.map=noiseMap([98,103,106],.22,10,{grain:2});asphalt.map.repeat.set(10,10);grass.bumpMap=grass.map;grass.bumpScale=.012;asphalt.bumpMap=asphalt.map;asphalt.bumpScale=.008;
function plankMap(repeatX,repeatY){const c=document.createElement('canvas');c.width=512;c.height=512;const g=c.getContext('2d');g.fillStyle='#d8d8d8';g.fillRect(0,0,512,512);for(let i=0;i<8;i++){const t=214+((i*7)%9);g.fillStyle=`rgb(${t},${t},${t})`;g.fillRect(0,i*64,512,61);g.fillStyle='rgba(50,50,50,.6)';g.fillRect(0,i*64+61,512,3);g.fillStyle='rgba(120,120,120,.18)';for(let k=0;k<5;k++)g.fillRect(0,i*64+6+k*11,512,1);g.fillStyle='rgba(60,60,60,.35)';g.fillRect((i*197)%512,i*64,2,61)}const t=new THREE.CanvasTexture(c);t.colorSpace=THREE.SRGBColorSpace;t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(repeatX,repeatY);t.anisotropy=renderer.capabilities.getMaxAnisotropy();return t}
...
const bark=mat('#5e4a3c',{roughness:1});bark.map=noiseMap([94,74,60],.5,3,{streaks:.08});
for(const [x,z,s]of [[-11,-14,2.8],[12,-22,3.1],[-12,-25,3.5],[13,-11,2.5]]){const trunk=new THREE.Mesh(new THREE.CylinderGeometry(.16*s/2.8,.26*s/2.8,5.2,10),bark);trunk.position.set(x,2.6,z);trunk.castShadow=true;scene.add(trunk);
  const seed=Math.abs(x*31+z*17)%97;
  for(const [ox,oy,oz,k]of [[0,0,0,1],[.55,.35,.2,.7],[-.5,.25,-.35,.65],[.1,.7,.45,.55],[-.2,-.3,.5,.6]]){const geo=new THREE.SphereGeometry(s*k,18,12),pos=geo.attributes.position;for(let i=0;i<pos.count;i++){const h=Math.sin(pos.getX(i)*12.9898+pos.getY(i)*78.233+pos.getZ(i)*37.719+seed)*43758.5453,j=.86+(h-Math.floor(h))*.28;pos.setXYZ(i,pos.getX(i)*j,pos.getY(i)*j*1.15,pos.getZ(i)*j)}geo.computeVertexNormals();const lobe=new THREE.Mesh(geo,mat(k===1?'#4f6a49':'#5b7a54',{roughness:.95}));lobe.position.set(x+ox*s,6+oy*s,z+oz*s);lobe.castShadow=true;scene.add(lobe)}}
